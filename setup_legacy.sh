#!/bin/bash
PATH_IDE_ROOT=`pwd`
PATH_LOCAL_VIM_ROOT=${HOME}"/.vim"
PATH_VIM_BACKUP=${PATH_IDE_ROOT}"/vim_bak_"$(date +%Y%m%d_%H%M%S)
...
function setup()
{
    ...
    echo ${PATH_VIM_BACKUP}
    if [ -d ${PATH_LOCAL_VIM_ROOT} ]
    then
        cp -rf ${PATH_LOCAL_VIM_ROOT} ${PATH_VIM_BACKUP}
    fi
    # if [ -e ${HOME}/.vimrc ]
    # then
    #     mv ${HOME}/.vimrc ${PATH_VIM_BACKUP}/
    # fi
    ...
    test -d ${PATH_LOCAL_VIM_ROOT}             || mkdir ${PATH_LOCAL_VIM_ROOT}
    test -L ${PATH_LOCAL_VIM_ROOT}/vim-ide.vim || ln -sf ${PATH_IDE_ROOT}/vim-ide.vim ${PATH_LOCAL_VIM_ROOT}/
    test -L ${PATH_LOCAL_VIM_ROOT}/vim-ide     || ln -sf ${PATH_IDE_ROOT}/scripts ${PATH_LOCAL_VIM_ROOT}/vim-ide
    test -L ${PATH_LOCAL_VIM_ROOT}/tools       || ln -sf ${PATH_IDE_ROOT}/tools ${PATH_LOCAL_VIM_ROOT}/tools
    test -L ${PATH_LOCAL_VIM_ROOT}/scripts     || ln -sf ${PATH_IDE_ROOT}/scripts ${PATH_LOCAL_VIM_ROOT}/scripts

    if [ ${flag_ln_all_plugins} = true ]
    then
        test -d ${PATH_LOCAL_VIM_ROOT}/plugins && rm -r ${PATH_LOCAL_VIM_ROOT}/plugins
        ln -sf ${PATH_IDE_ROOT}/plugins ${PATH_LOCAL_VIM_ROOT}/plugins
    else
        mkdir -p ${PATH_LOCAL_VIM_ROOT}/plugins/
        ln -sf ${PATH_IDE_ROOT}/plugins/* ${PATH_LOCAL_VIM_ROOT}/plugins/
    fi

    mkdir -p ${PATH_LOCAL_VIM_ROOT}/autoload/
    mkdir -p ${PATH_LOCAL_VIM_ROOT}/colors/
    mkdir -p ${PATH_LOCAL_VIM_ROOT}/swp/
    # patch for accerate startup speed
    # ln -sf ${PATH_LOCAL_VIM_ROOT}/plugins/vim-plug/plug.vim ${PATH_LOCAL_VIM_ROOT}/autoload/
    ln -sf ${PATH_LOCAL_VIM_ROOT}/plugins/vim-plug/plug.vim ${PATH_LOCAL_VIM_ROOT}/autoload/
    setup_cus_config
...
function setup_nvim()
{
    ...
    then
        ...
        then
            ...
            # vim things
            ln -sf ${PATH_IDE_ROOT}/tools ${var_nvim_config_root}/
            ln -sf ${PATH_IDE_ROOT}/scripts ${var_nvim_config_root}/scripts
        else
            return 0
        fi
    else
        ...
        # vim things
        ln -s ${PATH_IDE_ROOT}/tool ${var_nvim_config_root}/
        ln -s ${PATH_IDE_ROOT}/scripts ${var_nvim_config_root}/scripts
    fi
}
function setup_lite()
{
...
{
    touch ${PATH_LOCAL_VIM_ROOT}/ConfigCustomize.vim
    for each_config in $(cat scripts/core/Config.vim | grep let |tr -s ' ' | cut -d ':' -f 2 | cut -d '=' -f 1 | sort | uniq)
    do
        echo "Checking ${each_config}"
        if cat ${PATH_LOCAL_VIM_ROOT}/ConfigCustomize.vim | grep ${each_config} > /dev/null
        then
            echo " - Skip config ${each_config}, already exist"
        else
            echo " - Adding config ${each_config}"
            cat scripts/core/Config.vim | grep ${each_config} | head -n 1 | grep "\"n\"" | sed 's/get.*/"n"/g' >> ${PATH_LOCAL_VIM_ROOT}/ConfigCustomize.vim
            cat scripts/core/Config.vim | grep ${each_config} | head -n 1 | grep "\"y\"" | sed 's/get.*/"y"/g' >> ${PATH_LOCAL_VIM_ROOT}/ConfigCustomize.vim
        fi
    done
}
