...
function! s:Test_SetWindowTitle_sets_winbar_on_nvim() abort
    " On Neovim the sidebar title bar is a real winbar; on Vim it falls back
    " to a statusline. Either way the bar is set and highlighted. The global
    " 'laststatus' must NOT be touched (it is global in nvim — changing it
    " would kill the statusline for every window).
    new
    call LLMAgent_SetWindowTitle('LLM Chat', 'model: x')
    if has('nvim')
        call s:Assert(stridx(&winbar, 'LLMAgent_WinBar') >= 0, 'nvim winbar references the render callback')
        call s:Assert(stridx(&winhighlight, 'LLMAgentHiTitle') >= 0, 'nvim winbar uses the title highlight')
        call s:AssertEq(&l:statusline, ' ', 'nvim panel statusline is a blank space (no default fallback)')
    else
        call s:Assert(stridx(&statusline, 'LLM Chat') >= 0, 'vim statusline carries the title')
        call s:Assert(stridx(&statusline, 'LLMAgentHiTitle') >= 0, 'vim statusline uses the title highlight')
    endif
    call s:Assert(&laststatus != 0, 'global laststatus is left untouched')
    bwipe!
endfunction
...
function! s:Test_SlashCommand_agent_switches() abort
    " /agent <name> switches the persona; /agent with no arg keeps it.
    call LLMAgent_SetAgent('assistant')
    call LLMAgent_HandleSlashCommand('/agent debugger')
    call s:Assert(stridx(LLMAgent_GetSystemPrompt(), 'debugging') >= 0, '/agent debugger switches persona')
    call LLMAgent_HandleSlashCommand('/agent')
    call s:Assert(stridx(LLMAgent_GetSystemPrompt(), 'debugging') >= 0, '/agent with no arg keeps current')
    call LLMAgent_SetAgent('assistant')
endfunction

function! s:Test_SlashCommand_unknown_shows_help() abort
    " An unknown /command logs an error and the help list into the chat.
    silent! execute 'bwipe! LLMAgent-Chat'
    silent! execute 'bwipe! LLMAgent-Input'
    call LLMAgent_SidebarOpen()
    call LLMAgent_HandleSlashCommand('/bogus')
    let l:chat = bufnr('LLMAgent-Chat')
    let l:joined = join(getbufline(l:chat, 1, '$'), "\n")
    call s:Assert(stridx(l:joined, 'Unknown command') >= 0, 'unknown command is reported')
    call s:Assert(stridx(l:joined, 'Slash commands') >= 0, 'help list is shown')
    silent! execute 'bwipe! LLMAgent-Chat'
    silent! execute 'bwipe! LLMAgent-Input'
endfunction

function! s:Test_SlashHelp_renders_compact_block() abort
    " The help is one block with a single [System] tag and short lines, so it
    " fits the narrow sidebar without wrapping.
    silent! execute 'bwipe! LLMAgent-Chat'
    silent! execute 'bwipe! LLMAgent-Input'
    call LLMAgent_SidebarOpen()
    ...
    call s:Assert(stridx(l:joined, 'Slash commands:') >= 0, 'help header present')
    call s:Assert(stridx(l:joined, '/agent [name]') >= 0, 'agent command listed')
    call s:Assert(stridx(l:joined, '/help') >= 0, 'help command listed')
    " Only one [System] tag for the whole block.
    let l:tags = 0
    for l:ln in l:lines
        if stridx(l:ln, '[System]') >= 0
            let l:tags += 1
        endif
    endfor
    call s:AssertEq(l:tags, 1, 'help is a single tagged block')
    silent! execute 'bwipe! LLMAgent-Chat'
    silent! execute 'bwipe! LLMAgent-Input'
endfunction

function! s:Test_Agent_switch_changes_prompt_and_title() abort
    " Default agent is assistant; switching to debugger changes the system
    " prompt and the chat title bar.
    call LLMAgent_SetAgent('assistant')
    call s:Assert(stridx(LLMAgent_GetSystemPrompt(), 'coding assistant') >= 0, 'assistant prompt is the default')
    call LLMAgent_SetAgent('debugger')
    call s:Assert(stridx(LLMAgent_GetSystemPrompt(), 'debugging') >= 0, 'debugger prompt is used after switch')
    call LLMAgent_SetAgent('reviewer')
    call s:Assert(stridx(LLMAgent_GetSystemPrompt(), 'code reviewer') >= 0, 'reviewer prompt is used after switch')
    " Unknown agent is rejected and leaves the current agent unchanged.
    call LLMAgent_SetAgent('nope')
    call s:Assert(stridx(LLMAgent_GetSystemPrompt(), 'code reviewer') >= 0, 'unknown agent is rejected')
    call LLMAgent_SetAgent('assistant')
endfunction

function! s:Test_Agent_cycle_wraps_through_roles() abort
    " Shift+Tab cycles assistant -> debugger -> reviewer -> assistant.
    call LLMAgent_SetAgent('assistant')
    call LLMAgent_CycleAgent()
    call s:Assert(stridx(LLMAgent_GetSystemPrompt(), 'debugging') >= 0, 'cycle 1 -> debugger')
    call LLMAgent_CycleAgent()
    call s:Assert(stridx(LLMAgent_GetSystemPrompt(), 'code reviewer') >= 0, 'cycle 2 -> reviewer')
    call LLMAgent_CycleAgent()
    call s:Assert(stridx(LLMAgent_GetSystemPrompt(), 'coding assistant') >= 0, 'cycle 3 wraps to assistant')
    call LLMAgent_SetAgent('assistant')
endfunction

function! s:Test_SidebarNextPanel_toggles_between_panels() abort
    " Tab jumps between the two sidebar panels and only acts there.
    silent! execute 'bwipe! LLMAgent-Chat'
    silent! execute 'bwipe! LLMAgent-Input'
    call LLMAgent_SidebarOpen()
    let l:chat = bufnr('LLMAgent-Chat')
    let l:input = bufnr('LLMAgent-Input')
    " SidebarOpen leaves focus on the input panel.
    call s:AssertEq(bufnr('%'), l:input, 'focus starts on the input panel')
    call LLMAgent_SidebarNextPanel()
    call s:AssertEq(bufnr('%'), l:chat, 'Tab jumps to the chat panel')
    call LLMAgent_SidebarNextPanel()
    call s:AssertEq(bufnr('%'), l:input, 'Tab jumps back to the input panel')
    silent! execute 'bwipe! LLMAgent-Chat'
    silent! execute 'bwipe! LLMAgent-Input'
endfunction
...
function! s:Test_Commands_new_defined_old_removed() abort
    ...
    " matching, so compare against the exact :command list instead.
    let l:defined = map(split(execute('command'), "\n"), 'matchstr(v:val, ''^\s*\S*:\zs\S*'')')
